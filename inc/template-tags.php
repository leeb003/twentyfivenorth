...
function twentyfivenorth_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && twentyfivenorth_categorized_blog() ) {
			printf( '<p class="post-categories">' . esc_html__( 'Categories', 'twentyfivenorth' ) . ': ' . $categories_list . '</p>' ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'twentyfivenorth' ) );
		if ( $tags_list ) {
			printf( '<p class="post-tags">' . esc_html__( 'Tags', 'twentyfivenorth' ) . ': ' . $tags_list . '</p>' ); // WPCS: XSS OK.
		}
		printf( '</div>' );
	}
